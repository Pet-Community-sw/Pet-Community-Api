...
### 4. 채팅방 온라인 상태 관리 동시성 문제 개선

실시간 채팅 기능에서는 사용자의 채팅방 접속 상태를 기준으로 알림 발송 여부를 판단해야 했습니다.
채팅방에 접속 중인 사용자는 실시간 메시지를 바로 확인할 수 있기 때문에 알림 대상에서 제외하고, 접속 중이지 않은 사용자에게만 알림을 발송하도록 처리했습니다.

초기에는 HashMap을 사용해 채팅방별 온라인 사용자를 관리했습니다.
하지만 실시간 채팅 환경에서는 여러 사용자의 입장과 퇴장이 동시에 발생하기 때문에 일반 HashMap과 HashSet으로는 온라인 상태가 유실될 수 있다고 판단했습니다.
예를 들어 같은 채팅방에 두 사용자가 동시에 입장할 경우 두 스레드가 모두 기존 사용자 목록이 없다고 판단해 각각 다른 HashSet을 생성할 수 있습니다.
이후 마지막에 저장된 값이 이전 값을 덮어쓰면 일부 사용자의 온라인 상태가 저장되지 않는 문제가 발생할 수 있습니다.

이를 해결하기 위해 다음과 같은 방식으로 구조를 개선했습니다.

- ConcurrentHashMap을 활용해 채팅방별 온라인 사용자 상태 관리
- ConcurrentHashMap.newKeySet()을 사용해 사용자 목록도 동시 접근에 안전한 Set으로 구성
- 사용자 추가 시 compute()를 사용해 사용자 집합 생성과 조회 흐름을 안전하게 처리
- 사용자 제거 시 computeIfPresent()를 사용해 존재하는 채팅방에 대해서만 제거 로직 수행
- 온라인 사용자가 모두 나간 채팅방은 저장소에서 제거해 불필요한 데이터 유지 방지

이 구조를 통해 여러 사용자가 동시에 채팅방을 구독하거나 구독 해제하는 상황에서도 채팅방별 온라인 상태를 더 안전하게 관리할 수 있도록 개선했습니다.

---

### 5. 메시지 처리 구조의 분기 복잡도 개선
...
### 6. 게시글 목록 조회 시 연관 데이터 조회 과정에서 발생한 JPA N+1 문제
...
### 7. S3 직접 접근 구조를 CloudFront 기반 캐싱 구조로 개선
